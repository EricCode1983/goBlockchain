...
func handleConn(conn net.Conn) {
	...
	for scanBalance.Scan() {
		...
		t := time.Now()
		address = block.CalculateHash(t.String())
	...
	go func() {
		for {
			...
			for scanBPM.Scan() {
				...
				newBlock, err := block.GenerateBlockForPos(oldLastIndex, bpm, address)
				if err != nil {
					log.Println(err)
					continue
				}
...
func main() {
	...
	genesisBlock = block.Block{0, t.String(), 0, block.CalculateBlockHash(genesisBlock), "", ""}
